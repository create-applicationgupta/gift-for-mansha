...
Browser (Mansha / you)
    ...
    └─ Firestore reads/writes
           ...
           └─ collection: photos  (up to 5 images as compressed data)
```
...
4. Uploading a photo compresses it in the browser, then saves it to Firestore. If there are already 5, the oldest document is deleted first.
...
4. **Photos:** upload an image → should show in the grid; 6th upload removes oldest
...
| `src/lib/photos.ts` | Photo compress + upload + 5-cap rotation |
